use std::{fs::File, cmp::min, sync::{Arc, mpsc::Receiver}, thread, time::Duration, time::Instant};
...
fn wait_for_tick(start_time: Instant) {
    ...
    println!("{:>8} since last tick", since_last_tick / 1000);
    println!("{:>8} until next tick", until_next_tick / 1000);
    println!("");
